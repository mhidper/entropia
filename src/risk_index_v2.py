...
# ── Reproducibilidad: fijar todas las seeds ANTES del training ────────────
import random
random.seed(42)
np.random.seed(42)
...
train_res_std = {m: {} for m in ['VAR', 'ARIMA', 'RF', 'LSTM', 'DFM']}
...
for i, target in enumerate(EVAL_RANGE):
    ...
    try:
        ...
        idx_pib = list(var_data.columns).index('PIB')
        train_res_std['VAR'][target] = np.std(fitted_var.resid.iloc[:, idx_pib])
    ...
    try:
        ...
        train_res_std['ARIMA'][target] = np.std(fitted_ar.resid)
    ...
    try:
        ...
        rf_train_preds = rf.predict(X_rf_sc)
        train_res_std['RF'][target] = np.std(rf_train_preds - y_rf)
    ...
    try:
        ...
        lstm_train_preds = model_lstm.predict(X_lstm, verbose=0)
        res_lstm = scaler_lstm.inverse_transform(lstm_train_preds) - scaler_lstm.inverse_transform(y_lstm.reshape(-1, 1))
        train_res_std['LSTM'][target] = np.std(res_lstm)
    ...
    try:
        ...
        try:
            dfm_train_preds = fitted_dfm.fittedvalues.iloc[:, pib_idx]
            dfm_train_preds_unsc = dfm_train_preds * scaler_dfm.scale_[pib_idx] + scaler_dfm.mean_[pib_idx]
            train_res_std['DFM'][target] = np.std(dfm_train_preds_unsc - dfm_data['PIB'])
        except Exception:
            train_res_std['DFM'][target] = np.nan
...
ax.set_title('Five-Model Ensemble Forecasts vs Realized GDP (2015Q1-2024Q4)',
             fontsize=12)
...
fig.suptitle('Absolute Forecast Errors by Model (2015Q1-2024Q4)',
             fontsize=12, y=1.01)
plt.tight_layout()
...
ax.set_title('(A) MAE ratio: Crisis / Normal', fontsize=11)
...
fig.suptitle('Model Resilience Analysis - Post-Crisis Structural Effects',
             fontsize=12)
plt.tight_layout()
...
# ── Tabla 1 Panel A eliminada de aquí (se genera al final del módulo 4 con CEUI) ──────────
...
for m in models:
    ...
    rows_t4.append({
        ...
        'MAE Normal' : round(mae_pre_v, 3),
        'MAE Crisis': round(mae_pos_v, 3),
...
# ── Métrica de robustez: U_within_train ───────────────────────────────────────
u_within_train_df = pd.DataFrame({m: pd.Series(train_res_std[m]) for m in train_res_std})
u_within_train_df.index = pd.PeriodIndex(u_within_train_df.index, freq='Q')
u_within_train = u_within_train_df.mean(axis=1)
u_within_train.name = 'U_within_train'
dims_df['U_within_train'] = u_within_train

# ── CEUI en bruto (válido en tiempo real) ─────────────────────────────────────
dims_df['CEUI'] = dims_df[['U_within', 'U_between', 'U_temporal']].mean(axis=1)

# ── CEUI alternativo (Robustez con U_within_train) ────────────────────────────
dims_df['CEUI_robust_uwithin'] = dims_df[['U_within_train', 'U_between', 'U_temporal']].mean(axis=1)

# ── Normalización por percentil rank (0-100) a posteriori (descriptivo) ──────
dims_norm = dims_df.copy()
for col in ['U_within', 'U_between', 'U_temporal', 'CEUI']:
    dims_norm[col] = 100 * (dims_df[col].rank() - 1) / (len(dims_df) - 1)

print('\n=== DIMENSIONES EN BRUTO (REAL-TIME) ===')
print(dims_df.describe().round(3))

# ── Umbrales empíricos RECURSIVOS (para regímenes) ────────────────────────────
# Para uso en tiempo real, umbrales calculados con ventana expansiva (min 12 trimestres)
threshold_elevated_series = dims_df['CEUI'].expanding(min_periods=12).quantile(0.60)
threshold_high_series     = dims_df['CEUI'].expanding(min_periods=12).quantile(0.90)

# Umbral estático (para el resumen descriptivo final) calculado sobre dims_df['CEUI']
...
raw_within = dims_df['U_within']
raw_between = dims_df['U_between']
...
fig.suptitle('CEUI and its Three Dimensions (2015Q4-2024Q4)',
             fontsize=12, y=1.01)
plt.tight_layout()
...
ax.set_xlabel('Quarter', fontsize=11)
ax.set_title('CEUI Uncertainty Regimes (2015Q4-2024Q4)', fontsize=12)
ax.set_ylim(0, 20)
...
for regime in ['Normal', 'Elevated', 'High']:
    ...
    rows_t5.append({
        ...
        'N (CEUI)'    : n_ceui,
        'N (rev)'     : n_rev,
        'MAE rev (pp)': f"{mae_mean:.3f}" if not np.isnan(mae_mean) else '-',
        'vs Normal'   : ratio,
    })
...
# ── Tabla 1: Panel A (Estadísticos) y Panel B (Correlaciones) ────────────
desc_cols = ['actual', 'VAR', 'DFM', 'ARIMA', 'RF', 'LSTM']
desc = forecasts_df[desc_cols].describe().loc[['mean','std','min','max','count']]
desc.index = ['Mean', 'Std. Dev.', 'Min', 'Max', 'Obs.']
desc = desc.rename(columns={'actual': 'GDP Realized', 'VAR': 'VAR Forecast', 'DFM': 'DFM Forecast', 'ARIMA': 'ARIMA Forecast', 'RF': 'Random Forest', 'LSTM': 'LSTM Forecast'})
desc['CEUI Index (Raw)'] = ceui.describe().loc[['mean','std','min','max','count']].values
desc = desc.T

corr_models = ['VAR', 'RF', 'ARIMA', 'LSTM', 'DFM']
corr_matrix = forecasts_df[corr_models].corr(method='pearson')
...
for var_name, row in desc.iterrows():
    if var_name == 'CEUI Index (Raw)':
        latex_t1 += "\\midrule \n"
    latex_t1 += f"{var_name} & {row['Mean']:.2f} & {row['Std. Dev.']:.2f} & {row['Min']:.2f} & {row['Max']:.2f} & {int(row['Obs.'])} \\\\\n"
...
for i, row_name in enumerate(corr_models):
    latex_t1 += f"{row_name}"
    for j, col_name in enumerate(corr_models):
        if j < i:
...
with open(os.path.join(PATH_TABLES, 'tabla1_ensemble.tex'), 'w', encoding='utf-8') as f:
    f.write(latex_t1)
print('[OK] tabla1_ensemble.tex guardada')
...
# (5b) Spearman del indice NORMALIZADO -- robustez en tiempo real (seccion 7.1)
ceui_norm = dims_norm[['U_within', 'U_between', 'U_temporal']].mean(axis=1)
rho_norm, p_norm = spearmanr(ceui_norm.loc[common_idx], y)
print(f'  Spearman rho (CEUI normalizado): {rho_norm:.4f} (p={p_norm:.4f})')

# (5c) Spearman del CEUI con U_within_train (Robustez In-Sample)
ceui_robust = dims_df['CEUI_robust_uwithin'].loc[common_idx]
rho_robust, p_robust = spearmanr(ceui_robust, y)
print(f'  Spearman rho (CEUI con U_within_train): {rho_robust:.4f} (p={p_robust:.4f})')

# (5d) LOO para U_within_train (P5)
loo_rhos_robust = []
for excl in MODELS:
    remaining = [m for m in MODELS if m != excl]
    u_within_train_loo_df = pd.DataFrame({m: pd.Series(train_res_std[m]) for m in remaining})
    u_within_train_loo_df.index = pd.PeriodIndex(u_within_train_loo_df.index, freq='Q')
    u_within_train_loo = u_within_train_loo_df.mean(axis=1)
    ceui_robust_loo = pd.DataFrame({
        'U_within_train': u_within_train_loo,
        'U_between': dims_df['U_between'],
        'U_temporal': dims_df['U_temporal']
    }).mean(axis=1).loc[common_idx]
    rho_loo_rob, _ = spearmanr(ceui_robust_loo, y)
    loo_rhos_robust.append(rho_loo_rob)
min_loo_rob = min(loo_rhos_robust)
max_loo_rob = max(loo_rhos_robust)
print(f"  LOO U_within_train range: [{min_loo_rob:.3f}, {max_loo_rob:.3f}]")
...
# =============================================================================
# MODULE 5B -- Type I / Type II Uncertainty Diagnostic
# =============================================================================
# This module implements the empirical diagnostic proposed in the Framework 
# section to distinguish between "epistemic disagreement" (Type I) and 
# "systemic breakdown" (Type II) peaks.
# =============================================================================

print('\n=== MODULE 5B: TYPE I / TYPE II DIAGNOSTIC ===')

# 1. Calculating the Ratio U_within / U_between
# We use the raw (un-normalized) dimensions to maintain the physical scale
diag_data = pd.DataFrame({
    'U_within':  dims_df['U_within'],
    'U_between': dims_df['U_between'],
    'Ratio':     dims_df['U_within'] / dims_df['U_between'],
    'CEUI':      ceui,
    'Regime':    regime_series
})

# 2. Diagnostic Statistics by Regime
diag_stats = diag_data.groupby('Regime').agg({
    'Ratio': ['mean', 'std', 'max'],
    'CEUI': ['count']
})
print("\nRatio U_within / U_between by Regime:")
print(diag_stats.round(3))

# 3. Figure 8: Type I vs Type II Diagnostic Plot
fig, ax = plt.subplots(figsize=(11, 5))

# Plot the Ratio on secondary axis
ax_ratio = ax.twinx()
ln1 = ax.plot(diag_data.index.to_timestamp(), diag_data['CEUI'], 
             color='purple', linewidth=2.5, alpha=0.3, label='CEUI (Secondary)')
ln2 = ax_ratio.plot(diag_data.index.to_timestamp(), diag_data['Ratio'], 
                   color='black', linewidth=1.5, label='U_within / U_between')

# Highlight episodes where Ratio > 1.0 (Type II dominance)
mask_type2 = diag_data['Ratio'] > 1.0
ax_ratio.fill_between(diag_data.index.to_timestamp(), 0, diag_data['Ratio'], 
                     where=mask_type2, color='red', alpha=0.2, label='Type II dominance')
...
# Formatting
ax.set_ylabel('CEUI (Percentile Rank)', color='purple', fontsize=11)
ax_ratio.set_ylabel('Ratio $\\mathcal{U}^{within} / \\mathcal{U}^{between}$', fontsize=11)
ax_ratio.axhline(1.0, color='gray', linestyle='--', alpha=0.8)
ax.set_title('Uncertainty Diagnostic — Type I vs Type II', fontsize=12)
...
fig.savefig(f'{PATH_FIGURES}/fig8_type_diagnostic.pdf', bbox_inches='tight', dpi=300)
fig.savefig(f'{PATH_FIGURES}/fig8_type_diagnostic.png', bbox_inches='tight', dpi=300)
plt.close()

print(f'[OK] fig8_type_diagnostic guardada')
...
# Print normalized Spearman
dims_norm_aligned = dims_norm['CEUI'].reindex(sigma_rev.index).dropna()
sigma_rev_aligned = sigma_rev.reindex(dims_norm_aligned.index)
rho_norm, p_norm = spearmanr(dims_norm_aligned, sigma_rev_aligned)
print(f'CLAIMS CHECKER: Spearman rho (normalized index): {rho_norm:.3f}')
...
# Set seeds for reproducibility - MOVED TO MODULE 3B
...
fig.suptitle('Influence Diagnostics - CEUI vs $\\sigma_{rev}$',
             fontsize=12)
plt.tight_layout()
...
try:
    ...
    print("WARNING: VIX download failed, using NaNs")
    vix_q = pd.Series(index=pd.date_range('2015-01-01', '2024-12-31', freq='Q'), dtype=float)
...
try:
    ...
    print("WARNING: EPU download failed, using NaNs")
    epu_q = pd.Series(index=pd.date_range('2015-01-01', '2024-12-31', freq='Q'), dtype=float)
...
ax.set_title(
    f'Spearman Rank Correlation Matrix\n'
...
# ── Apéndice E: Horse-race regression CEUI vs VIX vs EPU ────────────────────
print('\n=== APPENDIX E: HORSE-RACE REGRESSIONS ===')

import statsmodels.api as sm

y_hr = corr_data['sigma_rev'] if 'sigma_rev' in corr_data.columns else corr_data.iloc[:, 1]

# Estandarizar predictores para comparabilidad
from sklearn.preprocessing import StandardScaler
pred_cols = ['CEUI', 'VIX', 'EPU']
scaler_hr = StandardScaler()
pred_std = pd.DataFrame(
    scaler_hr.fit_transform(corr_data[pred_cols]),
...
specs = {
    ...
    '(4) CEUI + VIX'    : ['CEUI', 'VIX'],
    '(5) CEUI + EPU'    : ['CEUI', 'EPU'],
    '(6) All three'     : ['CEUI', 'VIX', 'EPU'],
}

rows_hr = []
for spec_name, vars_list in specs.items():
    X = sm.add_constant(pred_std[vars_list])
    res = sm.OLS(y_hr, X).fit(cov_type='HC3')
    row = {'Specification': spec_name, 'N': len(y_hr), 'R2': round(res.rsquared, 3)}
    for v in pred_cols:
        if v in vars_list:
            stars = '***' if res.pvalues[v] < 0.01 else ('**' if res.pvalues[v] < 0.05 else ('*' if res.pvalues[v] < 0.1 else ''))
            row[f'b_{v}'] = f'{res.params[v]:.4f}{stars}'
            row[f't_{v}'] = f'{res.tvalues[v]:.2f}'
        else:
            row[f'b_{v}'] = '--'
            row[f't_{v}'] = '--'
    rows_hr.append(row)

table_hr = pd.DataFrame(rows_hr)
print(table_hr.to_string(index=False))

# LaTeX
latex_hr = (
    "\\begin{tabular}{lcccccccc}\n\\toprule\n"
    " & \\multicolumn{2}{c}{CEUI} & \\multicolumn{2}{c}{VIX}"
    " & \\multicolumn{2}{c}{EPU} & $R^2$ & $N$ \\\\\n"
    "Specification & $\\hat{\\beta}$ & $t$ & $\\hat{\\beta}$ & $t$"
    " & $\\hat{\\beta}$ & $t$ & & \\\\\n\\midrule\n"
)
for _, r in table_hr.iterrows():
    latex_hr += (f"{r['Specification']} & {r['b_CEUI']} & {r['t_CEUI']}"
                 f" & {r['b_VIX']} & {r['t_VIX']}"
                 f" & {r['b_EPU']} & {r['t_EPU']}"
                 f" & {r['R2']} & {r['N']} \\\\\n")
latex_hr += "\\bottomrule\n\\end{tabular}\n"

with open(f'{PATH_TABLES}/tab_horserace.tex', 'w', encoding='utf-8') as f:
    f.write(latex_hr)
print('Saved: tab_horserace.tex')


# =============================================================================
# CLAIMS CHECKER — Captura todos los valores citados en el paper
# =============================================================================
# Ejecutar DESPUÉS de todos los módulos del notebook.
# Los valores impresos aquí son la FUENTE DE VERDAD para el paper.
# =============================================================================

# ── GRUPO H: MAE, ratio y resiliencia por modelo ─────────────────────
print("\n[H] PER-MODEL MAE / RESILIENCE:")
for m, rr, mr in zip(models, resilience, mae_ratio):
    mae_full = (forecasts_df[m] - forecasts_df['actual']).abs().mean()
    print(f"   {m}: MAE_full={mae_full:.2f}pp, MAE_ratio={mr:.2f}x, resilience={rr:.3f}")

# ── GRUPO I: MAE estacional (Tabla 2) ────────────────────────────────
print("\n[I] SEASONAL MAE (Table 2):")
print(table2.to_string(index=False))

# ── GRUPO J: Rango del CEUI ──────────────────────────────────────────
print("\n[J] CEUI RANGE:")
print(f"   min={ceui.min():.2f} ({ceui.idxmin()}); max={ceui.max():.2f} ({ceui.idxmax()})")

# ── GRUPO K: Sensibilidad por submuestra (Tabla 7) ───────────────────
print("\n[K] SENSITIVITY TABLE (Table 7):")
print(table7.to_string(index=False))

# ── GRUPO L: Bootstrap (tabla completa) ──────────────────────────────
print("\n[L] BOOTSTRAP TABLE:")
print(table_thr.to_string(index=False))

# ── GRUPO M: Influencia (top 5) ──────────────────────────────────────
print("\n[M] INFLUENCE DIAGNOSTICS (top 5):")
print(diag_df.head(5).to_string())

# ── GRUPO N: Apéndice C (robustez por madurez) ───────────────────────
print("\n[N] APPENDIX C REGRESSIONS:")
print(f"   (1) baseline:   beta={res_c1.params.iloc[1]:.4f}, R2={res_c1.rsquared:.3f}, N={len(y_app)}")
print(f"   (2) +vint ctrl: beta_CEUI={res_c2.params['CEUI']:.4f}, "
      f"beta_vint={res_c2.params['vint']:.4f}, R2={res_c2.rsquared:.3f}")
print(f"   (3) excl young: beta={res_c3.params.iloc[1]:.4f}, R2={res_c3.rsquared:.3f}")

print('\n' + '=' * 70)
print('CLAIMS CHECKER — Valores fuente de verdad para main.tex')
# ── Escribir claims a claims_new.txt en la raíz (P0) ───────────────────
claims_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'claims_new.txt')
with open(claims_path, 'w', encoding='utf-8') as f:
    f.write(f"rho_baseline: {rho_s:.4f}\n")
    f.write(f"beta_baseline: {res1.params.iloc[1]:.4f}\n")
    f.write(f"se_baseline: {res1.bse.iloc[1]:.4f}\n")
    f.write(f"t_baseline: {res1.tvalues.iloc[1]:.3f}\n")
    f.write(f"p_baseline: {res1.pvalues.iloc[1]:.4f}\n")
    f.write(f"R2_baseline: {res1.rsquared:.4f}\n")
    f.write(f"rho_within_train: {rho_robust:.4f}\n")
    f.write(f"loo_within_train_min: {min_loo_rob:.4f}\n")
    f.write(f"loo_within_train_max: {max_loo_rob:.4f}\n")
    f.write(f"threshold_p60_elevated: {threshold_elevated:.2f}\n")
    f.write(f"threshold_p90_high: {threshold_high:.2f}\n")
    f.write("\n--- Sensitivity table (Table 7) ---\n")
    f.write(table7.to_string(index=False) + "\n")
    f.write("\n--- Horse-race table ---\n")
    f.write(table_hr.to_string(index=False) + "\n")
    f.write("\n--- Crisis Amplification (Table 3) ---\n")
    f.write(table3.to_string(index=False) + "\n")
    f.write("\n--- Regime Amplification (Table 5) ---\n")
    f.write(table5.to_string(index=False) + "\n")
    f.write("\n--- Weights Sensitivity (Table 6.1) ---\n")
    f.write(table_w.to_string(index=False) + "\n")
    f.write("\n--- LOO Sensitivity (Table 6.2) ---\n")
    f.write(table_loo.to_string(index=False) + "\n")
    f.write("\n--- Normalisation Sensitivity (Table 6.3) ---\n")
    f.write(table_norm.to_string(index=False) + "\n")

print('=' * 70)

# ── GRUPO A: Abstract y cifras principales ──────────────────────────
# A1. Spearman rho (full sample)
rho_full, p_val = spearmanr(ceui.loc[common_idx], sigma_rev.loc[common_idx])
print(f"A1. Spearman rho (full sample): {rho_full:.3f} (p={p_val:.4f})")

# A3. R2 full sample
X_ols = sm.add_constant(ceui.loc[common_idx])
res_ols_full = sm.OLS(sigma_rev.loc[common_idx], X_ols).fit(cov_type='HC3')
print(f"A3. R-squared (full sample):   {res_ols_full.rsquared:.3f}")

# A4. Beta coefficient (full)
print(f"A4. Beta coefficient (full):   {res_ols_full.params[1]:.4f}")

# A6. N common periods
print(f"A6. N periods available:       {len(common_idx)}")

# ── GRUPO B: Crisis Amplification (from table3) ──────────────────────
print("\n[B] CRISIS AMPLIFICATION FACTORS (Table 3):")
for _, r in table3.iterrows():
    if r['Period'] != 'Normal':
        print(f"   {r['Period']}: factor = {r['Amplif. Factor']}, MAE={r['MAE']:.3f}")

# ── GRUPO C: Regime Amplification (from table5) ──────────────────────
print("\n[C] REGIME AMPLIFICATION FACTORS (Table 5):")
for _, r in table5.iterrows():
    if r['Regime'] != 'Normal':
        print(f"   Regime {r['Regime']}: factor = {r['vs Normal']}, MAE={r['MAE rev (pp)']}")

# ── GRUPO D: CEUI Descriptivos ────────────────────────────────────
print("\n[D] CEUI THRESHOLDS & PEAKS:")
print(f"D1. CEUI Peak value:           {ceui.max():.2f} ({ceui.idxmax()})")
print(f"D3. Threshold p60 (Elevated):  {threshold_elevated:.2f}")
print(f"D4. Threshold p90 (High):      {threshold_high:.2f}")

# ── GRUPO E: Sensibilidad (from table7) ──────────────────────────
print("\n[E] SENSITIVITY RANGE (Table 7):")
betas = table7['beta'].values
print(f"E. Beta range: [{min(betas):.4f}, {max(betas):.4f}]")
print(f"E. R2 range:   [{table7['R²'].min():.3f}, {table7['R²'].max():.3f}]")

# ── GRUPO F: Influencia (from diag_df) ───────────────────────────
print("\n[F] INFLUENCE DIAGNOSTICS:")
print(f"F1. Most influential quarter:  {diag_df.index[0]}")
print("F2. Cook's D (max):           {:.3f}".format(diag_df.iloc[0]["Cook's D"]))

# ── GRUPO G: Bootstrap ────────────────────────────────────────────
print("\n[G] BOOTSTRAP STABILITY (95% CI):")
for _, r in table_thr.iterrows():
     print(f"   {r['Threshold']}: mean={r['Boot mean']:.2f}, CI=[{r['CI 2.5%']:.2f}, {r['CI 97.5%']:.2f}]")
